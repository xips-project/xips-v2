pipeline {
 ...
 /*        stage('Build') {
        ...
        stage('PIT Mutation') {
            steps {
                sh '/var/jenkins_home/tools/hudson.tasks.Maven_MavenInstallation/maven/bin/mvn -DwithHistory clean test-compile verify org.pitest:pitest-maven:mutationCoverage'
